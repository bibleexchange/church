...
App::singleton('oauth2', function() {
	...
	$storage = new App\OAuth2\Storage\Pdo(array('dsn' => 'mysql:dbname=dev_exchange;host=localhost', 'username' => 'root', 'password' => ''));
	$server = new App\OAuth2\Server($storage);
	
	$server->addGrantType(new App\OAuth2\GrantType\ClientCredentials($storage));
	$server->addGrantType(new App\OAuth2\GrantType\UserCredentials($storage));
