...
@section('office-content')
 ...
 {!! Form::open(array('url'=>'office/deposit', 'class'=>'well form-inline','role'=>'form','id'=>'gifts')) !!}
<h2>Deposit</h2>
...
<div class="panel panel-default">
  ...
  <div class="panel-heading"><a href="/office/deposit/{{$deposit->id}}">({{ $deposit->account->title }}) <strong>${{ $deposit->totalAmount() }}</strong> -  Deposited: {{ $deposit->deposited }}</a></div>
  <div class="panel-body">
    <p>
	@foreach ($deposit->offerings AS $offering)
		<a href="/office/offerings/{{$offering->id}}">{{ $offering->name }}</a> | 
	@endforeach
	</p>
  </div>
</div>
