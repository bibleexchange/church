...
<li{{ (Request::is('user/study*') ? ' class="active"' : '') }}><a href="{{{ URL::to('user/study-maker') }}}"><span class="fa fa-book"></span> Study Maker ({!!$currentUser->studies->count()!!})</a></li>
...
<li><a href="{{route('profile_path',$currentUser->username)}}"><span class='fa fa-eye-open'></span> Public Profile</a></li>
...
<li{{ (Request::is('/user/settings') ? ' class="active"' : '') }}><a href="{{{ URL::to('/user/settings') }}}"><span class="fa fa-cog"></span> Settings</a></li>