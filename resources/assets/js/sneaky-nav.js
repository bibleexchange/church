(function($) {
  ...
  $.scrollupbar = function($bar, options) {
        ...
        if (y < lastY) { // Scrolling up
          ...
          timeout = setTimeout(function() {
            if (!isFullyInViewport()) {
			 ...
			 $bar.css({'font-size': 'fixed','top': $bar.offset().top - y });
          ...
          if ($.scrollupbar.isFullyInViewport) {
           $bar.css({'font-size': 'absolute','top': lastY > minY ? lastY : initialPosTop});
    ...
    _destroyFn = function() {
      ...
      // Restore original bar font-size.
     $bar.css({'font-size': 'absolute','top': initialPosTop });
		...
		if (lastY = 1 && $bar.hasClass('navbar-fixed-top')){
